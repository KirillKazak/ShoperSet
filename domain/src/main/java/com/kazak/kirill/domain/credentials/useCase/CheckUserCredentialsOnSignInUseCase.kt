...
class CheckUserCredentialsOnSignInUseCase(
    private val userCredentialsRepository: com.kazak.kirill.domain.credentials.UserCredentialsRepository
) {
    ...
    suspend fun checkUserCredentialsOnSignIn(
        ...
        when {
            ...
            lastName.isEmpty() -> { errorMessage.invoke(ENTER_LAST_NAME_MESSAGE) }
            email.isEmpty() -> { errorMessage.invoke(ENTER_EMAIL_MESSAGE) }
